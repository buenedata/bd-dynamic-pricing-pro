...
if (!class_exists('BD_Dynamic_Pricing_Pro_Updater')) {
class BD_Dynamic_Pricing_Pro_Updater {
    private $plugin_file;
    ...
    public function __construct($plugin_file, $github_username, $github_repo) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("BD_Dynamic_Pricing_Pro_Updater initialized for {$this->plugin_basename} (slug: {$this->plugin_slug}, version: {$this->version})");
        }
    }
    ...
    public function plugin_info($result, $action, $args) {
        ...
        if (!is_wp_error($request) && wp_remote_retrieve_response_code($request) === 200) {
            ...
            $result = (object) [
                'name' => $data['name'] ?? 'BD Dynamic Pricing Pro',
                'slug' => $this->plugin_slug,
                ...
                'short_description' => 'BD Dynamic Pricing Pro fra Buene Data',
                'sections' => [
